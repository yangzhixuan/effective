...
-- | A varaint of `fuseR` that concatenates algebras rather than unions
-- algebras.  Concatenation is faster than union for most underlying data
...
-- | Compose the code of two functions such that @id . f@ and @f . id@ computes
-- to @f@. This is achieved by unsafe untyped Template Haskell API. There should
-- be safer way to do this but for now this version is sufficient.
...
-- | @passR r1 r2@ runs @r1@ and @r2@ in sequel but unlike @fuseR at2 r1 r2@,
-- in @passR r1 r2@, the effects produced by @r1@ is not handled by @at2@.
{-# INLINE passR #-}
passR
  :: forall effs2 oeffs1 oeffs2 ts1 ts2 a1 a2 a3 cs1 cs2.
     ( ForwardsC cs2 oeffs1 ts2
...
-- | A special case of @weakenRCSC@ for giving the type checking more hints.
weakenRCSCMonad
  :: forall ts2 effs ts a b.
     (forall m. Monad m => MonadApply ts2 m)
