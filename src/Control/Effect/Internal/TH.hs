...
@
where the arrow @~>@ signfies an effectful function. Note that this @~>@ is
only a syntax used by the Template Haskell helpers. The generated @put@
operation has type @put :: Member (Put s) effs => s -> Prog effs ()@ rather than
@put :: forall s. s ~> ()@.
...
-- | A dummy type that is used by the TH helpers to signifies 'effectful functions'.
data (~>) a b
infixr 0 ~>
...
-- $(makeScp [e| catch :: 2 |])
-- @
-- generates the operation @catch :: Member Catch effs => Prog effs x -> Prog effs x -> Prog effs x@ and
